...
class StudentRegisterForm(forms.Form):
    fullname = forms.CharField(
        label='Nama Lengkap', label_suffix=" : ", required=True, widget=forms.TextInput(
            attrs={'class': 'form-control', 'placeholder': 'John Smith'}),        help_text="Nama lengkap mahasiswa", error_messages={'required': "Harus Diisi"})
    nim = forms.CharField(
        label='Nim', label_suffix=" : ", required=True, widget=forms.TextInput(
            attrs={'class': 'form-control', 'placeholder': '102988444'}),        help_text="nomor induk mahasiswa", error_messages={'required': "Harus Diisi"})
    email = forms.CharField(
        label='email', label_suffix=" : ", required=True, widget=forms.TextInput(
            attrs={'class': 'form-control', 'placeholder': 'john@smith.co'}),        help_text="email aktif", error_messages={'required': "Harus Diisi"})
